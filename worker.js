...
export default {
  async fetch(request, env) {
    ...
    if(p.startsWith("/api/admin/")){
      ...
      const matchInscricao = p.match(/^\/api\/admin\/inscricoes\/(\d+)$/);
      if(matchInscricao && request.method==="PUT"){
        const id=Number(matchInscricao[1]);
        const b=await request.json().catch(()=>({}));
        const nome=String(b.nome||"").trim();
        const clube=String(b.clube||"").trim();
        if(!nome || !clube) return json({error:"Nome e clube são obrigatórios"},400);

        const exists=await env.DB.prepare("SELECT id FROM inscricoes WHERE id=?").bind(id).first();
        if(!exists) return json({error:"Inscrição não encontrada"},404);

        await env.DB.prepare("UPDATE inscricoes SET nome=?, clube=? WHERE id=?")
          .bind(nome.slice(0,160),clube.slice(0,160),id).run();

        return json({ok:true});
      }

      if(matchInscricao && request.method==="DELETE"){
        const id=Number(matchInscricao[1]);
        const exists=await env.DB.prepare("SELECT id FROM inscricoes WHERE id=?").bind(id).first();
        if(!exists) return json({error:"Inscrição não encontrada"},404);
        await env.DB.prepare("DELETE FROM inscricoes WHERE id=?").bind(id).run();
        return json({ok:true});
      }
